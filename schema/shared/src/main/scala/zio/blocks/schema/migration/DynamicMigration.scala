  ...
  private[migration] def applyAction(
      ...
      modifyAtPath(value, at) {
        case DynamicValue.Record(fields) =>
          if (fields.exists(_._1 == name)) {
            Left(MigrationError.single(MigrationError.FieldAlreadyExists(at, name)))
          } else {
            default.eval(value).map { defaultValue =>
    ...
    case MigrationAction.DropField(at, name, _) =>
      modifyAtPath(value, at) {
        case DynamicValue.Record(fields) =>
          if (!fields.exists(_._1 == name)) {
            Left(MigrationError.single(MigrationError.FieldNotFound(at, name)))
          } else {
      ...
      modifyAtPath(value, at) {
        case DynamicValue.Record(fields) =>
          if (!fields.exists(_._1 == from)) {
            Left(MigrationError.single(MigrationError.FieldNotFound(at, from)))
          } else if (fields.exists(_._1 == to)) {
            Left(MigrationError.single(MigrationError.FieldAlreadyExists(at, to)))
          } else {
    ...
    case MigrationAction.Join(at, sourcePaths, combiner, _) =>
      ...
      modifyAtPath(value, at.nodes.dropRight(1).foldLeft(DynamicOptic.root)(_ apply _.asInstanceOf[DynamicOptic])) {
        case record @ DynamicValue.Record(fields) =>
          ...
          if (sourceValues.length != sourcePaths.length) {
            ...
            combiner.eval(tempRecord).map { combined =>
    ...
    case MigrationAction.Split(at, targetPaths, splitter, _) =>
      modifyAtPath(value, at.nodes.dropRight(1).foldLeft(DynamicOptic.root)(_ apply _.asInstanceOf[DynamicOptic])) {
        case record @ DynamicValue.Record(fields) =>
          val sourceFieldName = at.nodes.lastOption match {
            case Some(DynamicOptic.Node.Field(name)) => name
            case _                                   => return Left(MigrationError.single(MigrationError.PathNavigationFailed(at, "Invalid path")))
          }
          fields.find(_._1 == sourceFieldName) match {
            case Some((_, sourceValue)) =>
              splitter.eval(sourceValue).flatMap {
                case DynamicValue.Sequence(splitValues) =>
                  if (splitValues.length != targetPaths.length) {
                    Left(MigrationError.single(MigrationError.ActionFailed(at, "Split", s"Expected ${targetPaths.length} values but got ${splitValues.length}")))
                  } else {
    ...
    case MigrationAction.TransformElements(at, transform, _) =>
      modifyAtPath(value, at) {
        case DynamicValue.Sequence(elements) =>
          val transformedElements = elements.map(transform.eval)
      ...
      path.nodes(idx) match {
        ...
        case DynamicOptic.Node.Wrapped =>
          value match {
            ...
            case other =>
              Left(MigrationError.single(MigrationError.PathNavigationFailed(path, s"Cannot unwrap ${getDynamicValueTypeName(other)}")))
          }

        case _: DynamicOptic.Node.AtIndices =>
          Left(MigrationError.single(MigrationError.ActionFailed(path, "AtIndices", "AtIndices traversal not supported in migration paths")))

        case _: DynamicOptic.Node.AtMapKeys =>
          Left(MigrationError.single(MigrationError.ActionFailed(path, "AtMapKeys", "AtMapKeys traversal not supported in migration paths")))
      }
    }
  }
